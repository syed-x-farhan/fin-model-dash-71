...
interface UseExcelImportReturn {
  ...
  handleImport: (data: RowData[], mappings: ColumnMapping[], fileName: string) => Promise<void>;
  clearImport: () => void;
...
export const useExcelImport = (): UseExcelImportReturn => {
  ...
  const handleImport = useCallback(async (
    ...
    try {
      ...
      setImportHistory(prev => [newImportedData, ...prev.slice(0, 9)]); // Keep last 10 imports
